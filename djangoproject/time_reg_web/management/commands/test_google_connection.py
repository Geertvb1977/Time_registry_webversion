    ...
    def handle(self, *args, **options):
        ...
        if not company_id:
            ...
            if not companies.exists():
                self.stdout.write(self.style.ERROR("[-] Er zijn geen bedrijven in de database gevonden. Voeg er eerst een toe."))
                return
            ...
            for c in companies:
                has_oauth = "JA" if hasattr(c, 'google_oauth_token') and c.google_oauth_token else "NEE"
                self.stdout.write(f"   [{c.id}] {c.name} (OAuth2 Tokens aanwezig: {has_oauth})")
            
            while True:
                try:
                    user_input = input("\nVoer het ID in van het bedrijf dat je wilt testen (of 'q' om te stoppen): ").strip()
                    ...
                    self.stdout.write(self.style.ERROR("[-] Ongeldige invoer. Voer een geldig numeriek ID in."))
                except Company.DoesNotExist:
                    self.stdout.write(self.style.ERROR(f"[-] Bedrijf met ID '{user_input}' bestaat niet. Probeer het opnieuw."))
        else:
            try:
                company = Company.objects.get(pk=company_id)
            except Company.DoesNotExist:
                self.stdout.write(self.style.ERROR(f"[-] Bedrijf met ID {company_id} bestaat niet."))
                return
        ...
        if not client_id_raw or not client_secret_raw or not token_info_raw:
            self.stdout.write(self.style.ERROR("[-] FOUT: Handmatige OAuth2-velden ontbreken nog in je Company database-model!"))
            self.stdout.write(self.style.WARNING("    Zorg ervoor dat de volgende velden op je Company-model staan en zijn ingevuld door de admin:"))
            self.stdout.write("    -> google_client_id = models.CharField(max_length=255, blank=True, null=True)")
            self.stdout.write("    -> google_client_secret = models.CharField(max_length=255, blank=True, null=True)  # Versleuteld opslaan!")
            self.stdout.write("    -> google_oauth_token = models.JSONField(blank=True, null=True)                  # Versleuteld opslaan!")
            return
        ...
        try:
            ...
            self.stdout.write(self.style.SUCCESS("    [v] OAuth2 configuratie succesvol geladen en gedecrypt."))
        except Exception as e:
            self.stdout.write(self.style.ERROR(f"[-] FOUT: Kan oauth_token JSON niet parsen/decrypten: {e}"))
            return
        ...
        try:
            ...
            if credentials.expired and credentials.refresh_token:
                self.stdout.write(self.style.WARNING("    [!] Access token is verlopen. Token verversen via refresh_token..."))
                ...
                company.save()
                self.stdout.write(self.style.SUCCESS("    [v] Access token succesvol vernieuwd en opgeslagen."))

            drive_client = build('drive', 'v3', credentials=credentials)
            self.stdout.write(self.style.SUCCESS("    [v] Google OAuth2 client succesvol opgebouwd."))
        except Exception as e:
            self.stdout.write(self.style.ERROR(f"[-] FOUT tijdens initialiseren OAuth2 client: {e}"))
            return
        ...
        try:
            ...
            self.stdout.write(self.style.SUCCESS(f"    [v] Verbinding geslaagd! Bestanden in de Drive van de Admin:"))
            for f in files:
                self.stdout.write(f"        - {f['name']} ({f['mimeType']}) ID: {f['id']}")
                
        except HttpError as error:
            self.stdout.write(self.style.ERROR(f"[-] Google Drive API Fout bij het oplijsten: {error}"))
            return
        ...
        try:
            ...
            self.stdout.write(self.style.SUCCESS(f"    [v] Map succesvol aangemaakt onder het Admin-account! ID: {test_folder_id}"))
            ...
            self.stdout.write(self.style.ERROR(f"[-] Google Drive API Schrijffout bij map aanmaken: {error}"))
            return
        ...
        try:
            doc_file = drive_client.files().create(
                body=doc_metadata,
                fields='id'
            ).execute()
            doc_id = doc_file.get('id')
            self.stdout.write(self.style.SUCCESS(f"    [v] Google Doc succesvol aangemaakt op de Drive van de Admin! ID: {doc_id}"))
        ...
        try:
            sheet_file = drive_client.files().create(
                body=sheet_metadata,
                fields='id'
            ).execute()
            sheet_id = sheet_file.get('id')
            self.stdout.write(self.style.SUCCESS(f"    [v] Google Sheet succesvol aangemaakt op de Drive van de Admin! ID: {sheet_id}"))
        ...
        self.stdout.write("\n" + "="*50)
        self.stdout.write(self.style.SUCCESS("[SUCCESS] OAuth2 API-verbinding en rechten zijn 100% in orde!"))
